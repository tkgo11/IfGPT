...
class ChatbotTests(unittest.TestCase):
    ...
    def test_exact_lookup_with_case_space_and_punctuation_variants(self) -> None:
        stored_prompt = "how can i decide whether to ask for more time before agreeing to something"
        expected = self.conversations[stored_prompt]
        live_variant = "  How can I decide whether to ask for more time before agreeing to something?  "
    ...
    def test_basic_if_elif_responses(self) -> None:
        self.assertEqual(chatbot.respond("   ", self.conversations), "Please enter a message so I can respond.")
        self.assertEqual(chatbot.respond("hello!", self.conversations), "Hello. Ask me a question or describe something you would like help with.")
        self.assertEqual(chatbot.respond("Thanks", self.conversations), "You are welcome.")
        self.assertEqual(chatbot.respond("who are you?", self.conversations), "I am a small rule-based chatbot using a separate JSONL conversation file.")
        self.assertEqual(chatbot.respond("quit", self.conversations), "Goodbye.")
        self.assertTrue(chatbot.is_exit_command("QUIT!"))

    def test_generic_fallback(self) -> None:
        response = chatbot.respond("please compose a sonnet about an orbital telescope", self.conversations)
